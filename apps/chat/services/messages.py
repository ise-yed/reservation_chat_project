...
from apps.chat.services.realtime import broadcast_message_deleted, broadcast_new_message
...
def send_message(*, conversation, sender, msg_type=MessageType.TEXT, content="", attachment=None) -> Message:
    ...
    receiver = conversation.provider if sender == conversation.customer else conversation.customer

    preview = content[:50] + "..." if len(content) > 50 else content
    if msg_type != MessageType.TEXT:
        preview = f"یک فایل {msg_type} ارسال کرد."

    sender_name = getattr(sender, "full_name", "") or getattr(sender, "email", str(sender))



    publish_notification_event(
        event_name=NotificationEvent.CHAT_MESSAGE_RECEIVED,
        recipient=receiver,
        context={
